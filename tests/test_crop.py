...
def widths(boxes, border):
    return [box[2] - box[0] + border[2] + border[0] for box in boxes]


@pytest.fixture
def heights(boxes, border):
    return [box[3] - box[1] + border[3] + border[1] for box in boxes]


@pytest.fixture
def areas(widths, heights):
    return [(widths[i]) * (heights[i]) for i in range(len(widths))]


@pytest.fixture
def objects(boxes, border):
    return [
        crop.MovingObject(0, 0, i + 1, box, [], border) for i, box in enumerate(boxes)
    ]
...
def test_combine_boxes(objects, objects_placement, object_map):
    output_objects, img_shape = crop.combine_boxes(
        objects, heuristic=crop.MergeHeuristic.FIRST_FIT_DECREASING
    )
...
def test_adjust_predicted_to_object_placement(
    obj_id, predicted, expected, objects_with_placement
):
    assert (
        crop.adjust_predicted_to_object_placement(
            predicted=predicted, object=objects_with_placement[obj_id - 1]
        )
        == expected
    )
...
def test_translate_to_object_coordinates(
    obj_id, predicted, expected, objects_with_placement
):
    assert (
        crop.translate_to_object_coordinates(
            predicted=predicted, object=objects_with_placement[obj_id - 1]
        )
        == expected
    )
...
@pytest.mark.parametrize(
    ("predicted", "expected", "min_overlap"),
    [
...
def test_translate_to_frame_coordinates(
    predicted, expected, min_overlap, objects_with_placement, object_map
):
    coords = crop.translate_to_frame_coordinates(
        predicted=predicted,
        object_map=object_map,
        objects=objects_with_placement,
