...
parser.add_argument(
    "--hybrid", default=False, action="store_true", help="Use hybrid (MOG+Diff) method."
)
...
def get_mask(background, frame, kernel):
    ...
    current_threshold = cv.threshold(current_delta, 25, 255, cv.THRESH_BINARY)[1]
...
for frame_id in range(int(total_frames)):
    ...
    if args.show:
        ...
        cv.putText(
            frame,
            f"{int(frame_id)}/{int(total_frames)}",
            (15, 15),
        ...
        cv.putText(
            frame,
            f"MOG2: {1/avg_mog:.2f} fps ({avg_mog*1000:.2f}ms.)",
            (15, 30),
            cv.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 0, 0),
        ...
        cv.putText(
            frame,
            f"Mean: {1/avg_mean:.2f} fps ({avg_mean*1000:.2f}ms.)",
            (15, 45),
            cv.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 0, 0),
        ...
        cv.putText(
            frame,
            f"Hybrid: {1/avg_hybrid:.2f} fps ({avg_hybrid*1000:.2f}ms.)",
            (15, 60),
            cv.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 0, 0),
