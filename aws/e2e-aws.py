    ...
    for field, params in required_fields.items():
        ...
        for param, valtype in params.items():
            try:
                ...
                logger.error(
                    f"Missing parameter '{param}' on field '{field}' in config file"
                )
                raise e
...
def capture_aws(
    ...
    for stream in tqdm.tqdm(streams_list):
        ...
        motionDetector = motion.MotionDetector(
            background=background, min_area_contour=min_area
        )
...
def annotate(bucket, imgs_prefix, output_prefix, role):
    ...
    ts1 = time.time()
    logger.info(
        f"Images successfuly annotated in {ts1-ts0:.2f} seconds. Results stored in {output_path}"
    )
    return output_path
...
def generate_manifest(bucket, imgs_prefix, results_prefix, dataset_name, valid_classes):
    ...
    for obj in s3_objects:
        ...
        with io.BytesIO() as f:
            ...
            img_dict = {
                "source-ref": f"{batch_input}/{Path(filename).stem}",
                dataset_name: {},
            }
...
def generate_tfrecord(
    bucket, s3_manifest, output_prefix, valid_classes, container, role
):
    ts0 = time.time()
    ...
    label_map = {str(i): c for i, c in enumerate(valid_classes)}
    ...
    data_processor.run(
        ...
        inputs=[
            ProcessingInput(
                input_name="input", source=s3_manifest, destination=input_folder
            )
        ],
        outputs=[
            ProcessingOutput(
...
def main():
    ...
    args.add_argument(
        "--streams", nargs="+", default=None, help="List of streams to process"
    )
    args.add_argument(
        "--s3-prefix-key",
        default=None,
        type=str,
        help="S3 key prefix where data will be stored",
    )
    args.add_argument("--dataset-name", default=None, type=str, help="Dataset name")
    args.add_argument(
        "--valid-classes", nargs="+", default=None, help="List of valid classes"
    )
    args.add_argument(
        "--warmup",
        type=int,
        default=10,
        help="Number of frames to warmup motion before start capturing",
    )
    args.add_argument(
        "--framerate",
        type=int,
        default=1,
        help="Rate at which images are captured (in frames/second)",
    )
    args.add_argument(
        "--show", action="store_true", default=False, help="Show window with results"
    )
    args.add_argument(
        "-f",
        "--file",
