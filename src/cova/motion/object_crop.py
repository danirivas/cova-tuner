...
def merge(img, boxes: list, heuristic=MergeHeuristic.FIRST_FIT_DECREASING):
    ...
    while True:
        ...
        area = max(img_shape) * max(
            img_shape
        )  # img will be squared, so we compute area wrt larges dim
...
def combine_streams(
    frames, box_lists: list, heuristic=MergeHeuristic.FIRST_FIT_DECREASING
):
    objects = []
...
def combine_resize(
    frames: list, box_lists: list, roi_size: list = (100, 100), border_size: int = 10
):
    objects = []
    all_boxes = []
    for stream_id in range(len(frames)):
        ...
        for box in boxes:
            ...
            objects.append(
                MovingObject(stream_id, 0, len(objects), new_box, [], border)
            )
...
def adjust_predicted_to_object_placement(
    ...
    if (
        predicted[0] > object.inf_box[2]
        or predicted[1] > object.inf_box[3]
        or predicted[2] < object.inf_box[0]
        or predicted[3] < object.inf_box[1]
    ):
        return None
