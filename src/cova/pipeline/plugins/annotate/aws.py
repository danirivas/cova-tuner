...
class AWSAnnotation(COVAAnnotate):
    ...
    def __init__(self, aws_config: dict, s3_config: dict):
        ...
        s3_config["images_prefix"] = s3_config.get(
            "images_prefix", default_images_prefix
        )
        s3_config["images_full"] = "s3://{}/{}".format(
            s3_config["bucket"], s3_config["images_prefix"]
        )

        # The location to store the results of the batch transform job
        default_annotations_prefix = os.path.join(s3_config["prefix"], "annotations")
        s3_config["annotations_prefix"] = s3_config.get(
            "annotations_prefix", default_annotations_prefix
        )
        s3_config["annotations_full"] = "s3://{}/{}".format(
            s3_config["bucket"], s3_config["annotations_prefix"]
        )
    ...
    def upload_image(
        self, img: np.array, filename: str, encoding: str = "PNG", to_rgb: bool = True
    ):
        """Uplaods image to s3."""
    ...
    def annotate_sagemaker(self) -> str:
        ...
        endpoint_name = self.aws_config.get(
            "endpoint_name", "{}-gt-endpoint".format(model_name)
        )

        model_arn = getattr(
            ModelPackageArnProvider, "get_{}_model_package_arn".format(model_name)
        )(region)
        ...
        output_path = batch_transform(
            data=self.s3_config["images_full"],
            transformer=batch,
            batch_output=self.s3_config["annotations_full"],
            content_type=content_type,
        )
        logger.info(
            "Images successfuly annotated in %.2f seconds. Results stored in %s",
            time.time() - ts0,
            output_path,
        )
        return output_path
